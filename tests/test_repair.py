...
def _rec(uid, *, status="Correct", my="A; Correct", key="A",
         question_text="Passage from JSON.", choices=(), snap=None,
         module="Module 1", num="1", test="SAT Practice Test 4",
         explanation="JSON rationale."):
    return {
        "uid": uid, "scraped_at": "2026-03-01T00:00:00+00:00", "test_name": test,
        "test_number": "4", "section": "Reading and Writing",
        "subject_bucket": "Reading and Writing", "module": module,
        "question_number": str(num), "domain": "", "skill": "",
        "my_answer": my, "correct_answer": key, "answer_status": status,
        "question_text": question_text,
        "answer_choices": list(choices), "explanation": explanation,
        "images": [], "html_snapshot_path": snap or "",
    }
...
def test_repair_reconciles_choice_less_rows_in_place(tmp_path, monkeypatch):
    ...
    with db_context(dbp) as conn:
        ...
        cur = conn.execute(
            """INSERT INTO questions (fingerprint, source, source_test, source_question_number,
               ...
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,0,'',?,?)""",
            (fp, "bluebook_test", rec["test_name"], rec["question_number"], rec["module"],
             rec["question_text"], "", "[]", "A", "", "[]", "", "", "unknown", "",
        ...
        conn.execute("INSERT INTO question_tags (question_id, tag, origin, created_at) "
                     "VALUES (?,?,?,?)", (qid, "manual_tag", "manual", "2026-01-01"))
    ...
    with db_context(dbp) as conn:
        ...
        assert conn.execute("SELECT COUNT(*) FROM attempts WHERE question_id=?", (qid,)).fetchone()[0] == 1
        assert conn.execute("SELECT COUNT(*) FROM question_tags WHERE question_id=?", (qid,)).fetchone()[0] == 1
...
def test_repair_records_both_layouts(tmp_path, monkeypatch):
    ...
    recs = [
        _rec("uid-c", snap=_abs_snap(tmp_path, snap_c), status="Correct", num="1", module="Module 1"),
        _rec("uid-i", snap=_abs_snap(tmp_path, snap_i), status="Incorrect", my="B; Incorrect", key="B",
             num="7", module="Module 2"),
    ]
    monkeypatch.setattr("satprep.config.BLUEBOOK_JSON", tmp_path / "outputs" / "wrong_questions.json")
