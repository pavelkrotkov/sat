...
from satprep.analytics import full_dashboard, next_action, recent_session_scores, session_comparison
...
def test_answering_leads_to_feedback_not_the_next_question(live):
    ...
    with db_context(live) as conn:
        ...
        response = server_mod.answer(
            None, sid, 0, question_id=qid, letter="B", confidence=3, elapsed_ms=1000, conn=conn
        )
...
def test_admin_stays_reachable_without_typing_a_url(live):
    ...
    with db_context(live) as conn:
        html = server_mod.templates.get_template("progress.html").render(d=full_dashboard(conn))
...
def test_session_rows_link_to_their_results_page(live):
    ...
    for html in (dashboard, progress):
        ...
        for sid in (sid1, sid2):
            link = re.search(rf'<a [^>]*href="/results/{sid}"[^>]*>(.*?)</a>', html, re.S)
...
def test_weakness_bars_are_scaled_to_the_risk_score_not_pinned_full(live):
    ...
    for width, tag in zip(risk_widths, d["tags"][:5], strict=False):
...
def test_dark_mode_follows_the_system_setting():
    ...
    assert (
        '<meta name="color-scheme" content="light dark">' in (TEMPLATES / "base.html").read_text()
    )
...
def test_completing_from_feedback_twice_is_harmless(live):
    ...
    with db_context(live) as conn:
        ...
        sid = sess["plan"]["session_id"]
        for _idx, q in enumerate(sess["questions"]):
            submit_answer(conn, sid, q["id"], "B", 3, 100)
...
def test_sessions_in_the_same_second_still_order_deterministically(live):
    ...
    with db_context(live) as conn:
        ...
        stamps = {
            r["attempted_at"] for r in conn.execute("SELECT attempted_at FROM attempts").fetchall()
        }
        anchor = conn.execute(
        ...
        previous = recent_session_scores(
            conn, exclude=ids[-1], before=(anchor["finished_at"], anchor["last_attempt_id"])
        )
...
def test_a_benchmark_never_shows_feedback_between_questions(live):
    ...
    with db_context(live) as conn:
        for i in range(10):
            add_question(
                conn, stem=f"protected {i}?", source="cb", pool="protected_benchmark", correct="B"
            )
        conn.commit()
...
def test_a_benchmark_still_reaches_its_results(live):
    ...
    with db_context(live) as conn:
        for i in range(10):
            add_question(
                conn, stem=f"pb-done {i}?", source="cb", pool="protected_benchmark", correct="B"
            )
        conn.commit()
    ...
    with db_context(live) as conn:
        ...
        for (qid,) in conn.execute("SELECT id FROM questions").fetchall():
            conn.execute(
                ...
                " VALUES (?,'scope_shift','rule','2026-01-01')",
                (qid,),
            )
        conn.commit()
