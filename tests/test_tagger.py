...
def test_wic_stem():
    skill, domain = derive_official_skill("As used in the text, what does the word 'grave' most nearly mean?")
    assert skill == "Words in Context"


def test_inference_stem():
    skill, _ = derive_official_skill("What does the text most strongly suggest about the migration?")
    assert skill == "Inferences"


def test_cross_text_stem():
    skill, _ = derive_official_skill("Based on the texts, how would the author of Text 1 most likely respond to Text 2?")
    assert skill == "Cross-Text Connections"


def test_generic_conventions_gets_domain_only():
    stem = "Which choice completes the text so that it conforms to the conventions of Standard English?"
    skill, domain = derive_official_skill(stem, choices=["one", "two; three", "four, five", "six"])
