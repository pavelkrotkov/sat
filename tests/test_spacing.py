...
def test_confident_wrong_is_soonest():
    intervals = {}
    for (correct, conf), (_, floor) in INTERVALS_BY_OUTCOME.items():
        iv, due = next_due(5.0, correct, conf, NOW)
...
def test_update_persists_state(db):
    ...
    interval = update_after_attempt(conn, qid, correct=0, confidence=3, now=NOW)
