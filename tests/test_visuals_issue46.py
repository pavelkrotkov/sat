...
def test_reimport_backfills_missing_visuals(db, fig_dirs):
    ...
    bare = {
        ...
        "domain": "Information and Ideas", "ext_id": "ext-table",
    }
...
def test_backfill_visuals_attaches_tables_to_visualless_rows(db, fig_dirs, monkeypatch):
    ...
    conn.execute("UPDATE questions SET images_json='[]', visuals_json='[]' "
...
def test_visuals_survive_archive_round_trip(db, tmp_path):
    ...
    conn.execute(
        ...
        (json.dumps([{"kind": "table", "html": "<table><tr><td>x</td></tr></table>"},
                     {"kind": "image", "file": "eqb-1-figure-1.svg"}]), qid),
    )
    conn.commit()
...
def test_drill_page_renders_a_table_visual(db, monkeypatch):
    ...
    conn, path = db
    qid = add_question(conn, passage="The table shows yields.",
    ...
    monkeypatch.setattr(server_mod, "db_context",
                        lambda db_path=None: db_context(path))
...
def test_extract_visuals_table_text_fallback_preserves_fingerprint(db, fig_dirs):
    ...
    legacy = "The table shows yields. Table 1. Yield by season Season Yield (t/ha) Spring 3.1 Summer 4.7"
