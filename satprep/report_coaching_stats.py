...
TIME_BUCKETS = ("not recorded", "<60 sec", "1:00–1:45", "1:45–2:30", ">2:30")
...
def _time_bucket(time_ms) -> str:
    ...
    if seconds <= 0:
        return TIME_BUCKETS[0]
    return TIME_BUCKETS[1 + bisect.bisect_right((60, 105, 150), seconds)]


def _coach(label: str) -> str:
    return {"<60 sec": _FAST_RULE, ">2:30": _SLOW_RULE}.get(label, "")
